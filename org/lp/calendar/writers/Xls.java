...
public class Xls extends Write{
            ...
            for (int gg = 0; gg < getGiornate().size(); gg++) {
                ...
                for (int i = 0; i < size; i++){
                    ...
                    if (singolo.getRiposa()==-1){
                        cell.setCellValue(new HSSFRichTextString(getSquadre().get(singolo.getCasa()-1)));
                        cell = row.createCell(1);
                        cell.setCellStyle(cs2);
                        cell.setCellValue(new HSSFRichTextString(getSquadre().get(singolo.getOspite()-1)));
                    } else {
                        cell.setCellValue(new HSSFRichTextString("Riposa:"));
                        cell = row.createCell(1);
                        cell.setCellStyle(cs2);
                        cell.setCellValue(new HSSFRichTextString(getSquadre().get(singolo.getRiposa()-1)));
                    }
                }
                row = sheet.createRow(++rownum);
            } //end for giornate
    ...
    private void initXLS() throws FileNotFoundException {
        // crea il file xls
